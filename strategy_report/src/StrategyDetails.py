...
class StrategyDetails:
        ...
        with SSHTunnelForwarder(
            ...
            try:
                ...
                with connection.cursor() as cursor:
                    ...
                    self.strategy_name = result[0][1].replace("- SDK", "")
        ...
        with SSHTunnelForwarder(
            ...
            try:
                ...
                with connection.cursor() as cursor:
                    ...
                    if result.empty:
                        #Pegar a posição do master
                        sql = f"""
                            SELECT position_time_current, position_type, investment_id, position_profit from tb_positions tp 
                            where investment_id in (SELECT investment_id from tb_investments ti where strategy_id = {self.strategy_id} and ti.follow_type = "MASTER");                        
                        """
                        cursor.execute(sql)
                        result = pd.DataFrame(cursor.fetchall(), columns=['datetime', 'position_type', 'investment_id', 'position_profit'])
    ...
    def create_strategy_performance(self):
        ...
        self.generate_benchmark(data_inicial=data.index[0].strftime("%d/%m/%Y"), data_final=data.index[-1].strftime("%d/%m/%Y"))
    ...
    def generate_benchmark(self, data_inicial=None, data_final=None):
...
if __name__ == "__main__":
    strategy = StrategyDetails(1378)
    strategy.create_strategy_performance()
